...
def send_chirp(chirp, client):
    serialized_packet = json.dumps(
        { "packet_type": "data", "data": { "frame": chirp} }
    )
    ...
    def send_config_packet(self, client_socket):
        serialized_config = json.dumps({
            ...
            "config": {
                ...
                "maximum_range": self.metrics.actual_max_range, 
                "minimum_range": self.metrics.min_range,
                "speed_resolution": self.metrics.speed_resolution,
                "maximum_speed": self.metrics.actual_max_velocity,
                "frame_rate": self.metrics.frame_rate,
                "adc_samplerate_hz": self.metrics.adc_sample_rate_hz,
                "bgt_tx_power": self.metrics.bgt_tx_power,
                "rx_antenna_number": self.metrics.rx_antenna_number,
                "if_gain_db": self.metrics.if_gain_db,
                "fmcw_center_frequency_khz": int(0.001*self.metrics.center_frequency),
                "lower_frequency_khz": int(0.001*self.metrics.lower_frequency),
                "upper_frequency_khz": int(0.001*self.metrics.upper_frequency),
                "range_fft_size": self.metrics.num_samples_per_chirp,
        ...
        string_msg = packet_len + serialized_config 
        msg = bytes(string_msg.encode())
